...
function HomeNavBar() {
  ...
  const navLinks = [
    ...
    { path: '/signUp', label: <span className='bg-green-700 px-4 py-2 rounded text-gray-200'>Sign Up</span> }
  ]
  ...
  return (
    <div className='flex justify-between items-center bg-gray-950 py-5 md:pl-24 pl-1'>
      ...
      <div className={`absolute md:top-0 top-16 right-16 ${isMenuOpen ? "flex" : "hidden"} md:relative md:flex flex-col md:flex-row items-center md:items-center md:pr-8 md:bg-transparent bg-gray-100 rounded p-4`}>
            ...
            {dropDownMenu.map((menu, index) => (
              <li
                ...
                <NavLink
                  ...
                  className={({isActive}) => (
                    isActive? "text-green-600 font-bold" : "text-gray-500 hover:text-gray-600"
                  )}
                  >
